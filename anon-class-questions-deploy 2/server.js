...
function normalizeDatabase(parsed) {
  ...
  database.questions = database.questions.map((question) => ({
    ...question,
    answerText: typeof question.answerText === 'string' ? question.answerText : ''
  }));
...
function statsForClass(classroom) {
...
async function handleApi(req, res, url) {
  ...
  if (classQuestionsMatch) {
    ...
    if (method === 'POST') {
      if (classroom.acceptingQuestions === false) {
        return sendJson(res, 403, { error: 'This board is not accepting new questions right now.' });
      }
  ...
  if (method === 'GET' && professorMatch) {
    ...
    const questions = questionRowsForClass(classroom.id, true);
    return sendJson(res, 200, { class: publicClassRow(classroom), questions, stats: statsForClass(classroom) });
  }

  const professorSettingsMatch = pathname.match(/^\/api\/professor\/([^/]+)\/settings$/);
  if (method === 'PATCH' && professorSettingsMatch) {
    const classroom = verifyProfessor(professorSettingsMatch[1], url.searchParams.get('token'));
    if (!classroom) return sendJson(res, 401, { error: 'Invalid professor link.' });

    const requestBody = await readRequestBody(req);
    if (typeof requestBody.acceptingQuestions !== 'boolean') {
      return sendJson(res, 400, { error: 'No valid setting provided.' });
    }

    classroom.acceptingQuestions = requestBody.acceptingQuestions;
    saveDatabase();

    return sendJson(res, 200, { class: publicClassRow(classroom), stats: statsForClass(classroom) });
  }
  ...
  if (professorQuestionMatch) {
    ...
    if (method === 'PATCH') {
      ...
      if (typeof requestBody.answerText === 'string') {
        const answerText = cleanText(requestBody.answerText, 800);
        question.answerText = answerText;
        if (answerText && question.status !== 'answered') {
      ...
      return sendJson(res, 200, { question: updated, stats: statsForClass(classroom) });
    }

    if (method === 'DELETE') {
      question.isHidden = true;
      saveDatabase();
      return sendJson(res, 200, { ok: true, stats: statsForClass(classroom) });
    }
  }
